...
\subsection{Layers}
\index{Layers}
To view the toolpath of your print head and to ensure no skipped layers or gaps use this option. Use the slide bar on the right hand side of the window to move up and down through the toolpath layers. If there is support activated in your model, it will be shown in blue.
\begin{figure}[H]
\centering
...
\begin{center}
 ...
 \hline
 N-Vent & Gluestick & 225-260 & 110 & 50 \\ [1ex]
 \hline
\end{tabular}
\end{center}
...
\begin{itemize}
\item Load the outer and inner calibrations squares into Cura \texttt{https://www.lulzbot.com/support/downloads} Order counts, load the outer square first.
\item Open the Control window and set temperatures for both heads. \texttt{Switch between hotends using T0 (rear) and T1 (front.)}
\item Make two marks at 100mm and 120mm on the filament from each entrance to the extruder.
\item Once at temperature type \textcolor{red}{Awaiting BAM's tested Gcode commands}
...
\item Update your Esteps (rear) and E1steps (front) through your LCD screen. \texttt{Control > Motion > E/E1 Steps > New Number (click)}
\item Store your new E steps \texttt{Control > Store Memory}
\end{itemize}
