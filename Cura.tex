...
\item Choose the correct nozzle size for your machine. Your TAZ 6 comes with a \texttt{0.5mm nozzle} standard.
...
The print quality settings can be found in the top left-hand corner of the window. For most filaments, there will be Standard, High Speed, and High Detail options. Some of the more exotic filaments may only have a single profile.
...
\index{filament}
We have the different filaments separated by \texttt{ease of use.} From the ease of use drop down, select \texttt{"all"} to have access to all our pre-loaded filament profiles. The TAZ ships with a 1 meter sample of nGen, that should be used in your first print.
...
\index{ABL}
Before each print your, TAZ 6 will go through a wiping and a probing procedure in order to determine the slope/tilt of your bed. If your nozzle is dirty or not cleaned properly, this will prevent the printer from being able to create an electrical contact with the corners. Your TAZ 6 will automatically retry the wiping and probing procedure if this happens, up to a maximum of three times. If three consecutive wipes and probes have failed, the printer will stop and a warning will sound. If this is happening consistently, you may need to replace the wiping pad and/or adjust your wiping and probing temps for that specific filament. See section \ref{sssec:num1} on page \pageref{sssec:num1} for details on changing these temps.
%\subsection{Mini}
...
\index{printing}
With your printer's automatic bed leveling, it is now time to kick off the print! Through the LCD screen go to: \texttt{Print From SD} > \texttt{Desired File}. Your TAZ 6 will then go through it's automatic cleaning and wiping process before starting the print. This should take approximately 5 minutes.
...
Connect your 3D printer to a computer using a USB cable, power it on and select the \texttt{Control} button at the top of the 3D viewing window. \texttt{If the control button is not present, be sure you have a model file loaded into Cura.} This will bring up your Pronterface user interface. You will not be able to send any commands until the window title changes to \texttt{Operational}. Once it is operational, you can start off your first print. Your TAZ 6 will then go through it's automatic cleaning and wiping process before starting the print. This should take approximately 5 minutes.
...
This is where you will set the hot end temperature for your specific material. It will be pre-set when using the recommended quickprint profiles. This can be adjusted for fine tuning of different filament manufacturers to help layer adhesion or stringing. See Recommended Temperatures Table: page \pageref{tab:a}. Your hot end is capable of reaching a maximum temperature of 300c.
...
This is where you will set the heat bed temperature for your specific material. These will be pre-set when using the recommended quickprint profiles. This can be adjusted for fine tuning of different filament manufacturers to help adhesion or release. See Recommended Temperatures Table: page \pageref{tab:a}. Your bed is capable of reaching a maximum temperature of 135c. 
...
By changing the variables \texttt{(XXX, YYY, ZZZ),} you can change what temperature your printer will soften, wipe, and probe. Our temperatures for softening, wiping, and probing will be a good starting point for other manufacturer's filament. When making adjustments, we recommend +/- 5c changes at a time. \texttt{Be sure to watch the probing sequence when experimenting with new temperatures.}
